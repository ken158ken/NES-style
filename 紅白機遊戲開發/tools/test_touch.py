...
涵蓋（三頁 × iPhone 13 橫向 / Pixel 5 直向 = 6 組，每組 18 項；另加桌機 1280×800 三頁 × 4 項）：
  版面：覆蓋層自動顯示、六顆按鍵齊全、按鍵矩形不與畫面交集、不出界、overlapping=false、
  ...
  自動顯示：鍵盤輸入後淡出、再觸控又出現
  桌機：不顯示觸控層、scale 整數 3、backing store 768×672、CSS 尺寸整數
"""
import argparse
import pathlib
import sys
...
def run_mobile(p, page_name, device, landscape, label, results):
    ...
    try:
        ...
        ok('六顆按鍵齊全（搖桿/A/B/SELECT/START/全螢幕）',
