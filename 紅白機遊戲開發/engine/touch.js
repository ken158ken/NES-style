...
(function (root) {
  ...
  function relayout() {
    ...
    var selP, staP, fsP;
    if (portrait) {
      // 一排置中放在主排上方：[SELECT][START][全螢幕]
      var totW = sw * 2 + fw + gap * 2;
      ...
      fsP = avoid(x0 + sw * 2 + gap * 2 + fw / 2, rowY, fw, fw, 'below', L);
    } else {
      // 橫向：SELECT / START 疊在動作鍵那一側的上方；全螢幕在搖桿那一側的上方
    ...
    if (portrait) {
      ...
      zy1 = bottom;
      if (actRight) zx1 = Math.min(zx1, Math.min(cluBox.x, selP.cx - sw / 2) - 4);
      else zx0 = Math.max(zx0, Math.max(cluBox.x + cluBox.w, staP.cx + sw / 2) + 4);
    } else {
  ...
  function bindButton(el, bit) {
    ...
    function up(e) {
      ...
      unlockAudio();
      if (!bit) toggleFullscreen();        // bit = 0 ⇒ 全螢幕鍵（不送遊戲按鍵）
  ...
  T.buttons = { dpad: pad, a: bA, b: bB, select: bSel, start: bStart, fs: bFs };
  T.overlapping = false;
  T.el = rootEl;
